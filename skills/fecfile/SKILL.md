...
The MCP server receives the FEC API key via environment variable (configured during MCPB installation and stored in system keychain). The API key is never visible to the model.
...
**If the MCP tools return a 403 or auth error:** The API key is missing or misconfigured. Do not fall back to direct API calls. Instead, inform the user that the FEC API key needs to be reconfigured in the MCPB settings, and ask them to restart the app. The key is stored in the system keychain and was configured during MCPB installation.
